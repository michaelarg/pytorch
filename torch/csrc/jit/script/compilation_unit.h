...
using Resolver = std::function<std::shared_ptr<
    SugaredValue>(const std::string& name, Function& f, const SourceRange& loc)>;
...
struct TORCH_API CompilationUnit {
  ...
  Function& create_function(std::string name, std::shared_ptr<Graph> graph) {
    std::unique_ptr<Function> fn{
        new Function(std::move(name), is_optimized(), std::move(graph), nullptr)};
...
private:
  ...
  Function& register_function(std::unique_ptr<Function> fn) {
    AT_CHECK(0 == dict_.count(fn->name()), "method '", fn->name(), "' already defined.");
